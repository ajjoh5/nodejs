...
function ensureRunOnce(req, res, next) {

    console.log('[ Run Once ]');
    var appConfigFile = path.join(__dirname + '/config/app.json');
    var appConfigJsonFile = fs.readFileSync(appConfigFile);
    var appJson = JSON.parse(appConfigJsonFile);

    if(!appJson) {
        res.status(500).send('ERROR: Could not load app config file.');
    }
    else {
        if(!appJson.IsRunning) {
            appJson.IsRunning = true;
            fs.writeFileSync(appConfigFile, JSON.stringify(appJson));
            next();
        }
        else {
            res.status(500).send('ERROR: Application is already running. Please try again later.');
        }
    }
}

function updateRunOnceToFalse(rangeName) {

    console.log('[ Upated Run Once | Range: ' + rangeName + ' ]');
    var appConfigFile = path.join(__dirname + '/config/app.json');
    var appConfigJsonFile = fs.readFileSync(appConfigFile);
    var appJson = JSON.parse(appConfigJsonFile);

    if(!appJson) {
        console.log('ERROR: Could not update app config for range: ' + rangeName)
    }
    else {
        var dateStamp = formatDate(new Date());

        var rangeConfig = _.find(appJson.Ranges, function(item) {
        ...
        if(!rangeConfig) {
            ...
            appJson.Ranges.push(rangeConfig);
        }

        //Update to running = false, LastRun for range to current date and time
        appJson.IsRunning = false;
        rangeConfig.LastRun = dateStamp;
        ...
        function(err, results){
            ...
            runwaydata.JoinPublishedAndRangePackages(results[0], results[1], rangeName, function(err, data) {
                ...
                async.parallel(promises, function(err, results) {
                    ...
                    res.send(data);
                    updateRunOnceToFalse(rangeName);
                });
